...
const BillModal = ({ isOpen, onClose, order, onRequestPayment }) => {
  ...
  const handlePaymentRequest = async () => {
    ...
    const confirmed = window.confirm(
      `Xác nhận thanh toán ${formatCurrency(order.total_amount)} bằng ${getPaymentMethodName(selectedPaymentMethod)}?`
    );
    ...
    try {
      ...
      if (selectedPaymentMethod === 'cash') {
        ...
        alert('✅ Đã gửi yêu cầu thanh toán tiền mặt. Vui lòng chờ nhân viên đến thu tiền.');
        onClose();
      } else {
        // Online payment: Mở cổng thanh toán (Mock)
        handleOnlinePayment(selectedPaymentMethod, order.id, order.total_amount);
      }
    } catch (error) {
  ...
  const handleOnlinePayment = (method, orderId, amount) => {
    const mockUrls = {
      ...
      'momo': `http://localhost:5000/api/customer/payment/momo-callback?orderId=${orderId}&status=success&transactionId=MOMO_${Date.now()}`,
    ...
    if (paymentUrl) {
      // Trong thực tế, đây sẽ redirect đến payment gateway
      // Hiện tại mock bằng cách mở tab mới rồi callback
      alert(`🔄 Đang chuyển đến cổng thanh toán ${getPaymentMethodName(method)}...\n\n(Mock: Sẽ tự động hoàn tất sau 2 giây)`);
      
      setTimeout(async () => {
        try {
          // Gọi complete payment
          const response = await fetch(`http://localhost:5000/api/customer/orders/${orderId}/complete-payment`, {
            method: 'POST',
  ...
  const subtotal = activeItems.reduce((sum, item) => {
    ...
    const modifierPrice = (item.modifiers || []).reduce((modSum, mod) => 
      modSum + parseFloat(mod.modifier_option?.price || 0), 0
    );
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <span className="font-semibold text-right">
                <span className={`px-2 py-1 rounded text-xs ${
                  order.status === 'payment' ? 'bg-purple-100 text-purple-700' : 'bg-gray-200 text-gray-700'
                }`}>
                  {order.status === 'payment' ? 'Chờ thanh toán' : order.status.toUpperCase()}
                </span>
              </span>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {activeItems.map((item, idx) => {
                ...
                const modifierPrice = (item.modifiers || []).reduce((sum, mod) => 
                  sum + parseFloat(mod.modifier_option?.price || 0), 0
                );
                        ...
                        <div className="mt-1">
                          <span className={`text-[10px] px-2 py-0.5 rounded ${
                            ...
                            {item.status === 'served' ? '✓ Đã lên' : 
                             item.status === 'ready' ? 'Chờ bưng' : 'Đang làm'}
                          </span>
                        </div>
                        ...
                        {item.modifiers && item.modifiers.length > 0 && (
                          <div className="text-xs text-gray-500 mt-1 ml-2">
                            {item.modifiers.map((mod, modIdx) => (
                              <div key={modIdx}>
                                + {mod.modifier_option?.name} (+{formatCurrency(mod.modifier_option?.price || 0)})
                              </div>
                            ))}
                          </div>
                        )}
          ...
          {!allItemsServed && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div className="text-sm text-yellow-800">
                ...
                <p>Vui lòng đợi tất cả món được phục vụ trước khi thanh toán.</p>
              </div>
            </div>
          )}
