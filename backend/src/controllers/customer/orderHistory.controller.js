...
export const createOrder = async (req, res) => {
    try {
        ...
        if (error) { //Trả vè nếu có lỗi về kiểu dữ liệu
        ...
        return res.status(201).json({ 
            success: true, 
            message: customerID ? "Đặt món thành viên thành công" : "Khách vãng lai đặt món thành công",
            data: order 
        });
        ...
        if (error.name === 'SequelizeForeignKeyConstraintError') {
            return res.status(400).json({
                ...
                message: "Mã bàn không tồn tại trong hệ thống. Vui lòng quét lại mã QR."
            });
        }
        ...
        if (error.name === 'SequelizeDatabaseError' && error.message.includes('not-null')) {
            return res.status(500).json({
                ...
                message: "Hệ thống chưa cho phép khách vãng lai đặt món (Cột customer_id đang bắt buộc)."
            });
        }
...
export const getMyOrders = async (req, res) => {
    try {
        ...
        const orders = await OrderService.getCustomerOrder(customerID);
