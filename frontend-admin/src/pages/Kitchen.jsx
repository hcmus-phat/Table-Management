...
const Kitchen = () => {
  ...
  const filteredOrders = orders
    .filter((o) => {
      ...
      if (['completed', 'cancelled', 'pending'].includes(o.status)) return false;
  ...
  if (loading) return <div className="h-screen flex items-center justify-center"><div className="animate-spin text-4xl">🍳</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-900">
        ...
        <div className="max-w-8xl mx-auto px-4 py-3 flex justify-between items-center">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-400">Đang hoạt động • {new Date().toLocaleTimeString('vi-VN')}</p>
                </div>
            </div>
            ...
            <div className="flex gap-3">
                 ...
                 <button onClick={handleRefresh} className="p-2 bg-blue-600 rounded-lg text-white hover:bg-blue-500">↻</button>
            </div>
        </div>
      </header>
      ...
      <div className="bg-white border-b sticky top-[64px] z-10 shadow-sm">
         <div className="max-w-8xl mx-auto px-4 flex overflow-x-auto gap-4 pt-3 pb-0">
            {[
                ...
                { key: 'pending', label: 'Chờ nấu', count: stats.pending, color: 'border-yellow-500 text-yellow-700' },
                { key: 'preparing', label: 'Đang nấu', count: stats.preparing, color: 'border-blue-500 text-blue-700' },
                { key: 'ready', label: 'Sẵn sàng', count: stats.ready, color: 'border-green-500 text-green-700' },
            ].map(tab => (
                <button 
                    key={tab.key}
      ...
      <main className="p-4 pb-24 max-w-8xl mx-auto">
         {error && <div className="bg-red-100 text-red-700 p-3 rounded mb-4 border border-red-300">⚠️ {error}</div>}
         ...
         <div className="mt-8 flex justify-center gap-6 text-xs text-gray-500">
             <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-green-500"></span> Mới (&lt;{WARNING_TIME}p)</span>
             <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-yellow-500"></span> Chậm ({WARNING_TIME}-{OVERDUE_TIME}p)</span>
             <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-red-500 animate-pulse"></span> Quá hạn (&gt;{OVERDUE_TIME}p)</span>
         </div>
      </main>
